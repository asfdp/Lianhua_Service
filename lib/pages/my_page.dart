...
class MyPage extends StatelessWidget {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [AppColors.accentColor, AppColors.assistantColor],
        ),
      ),
      child: Stack(
        children: [
  ...
  Widget _buildFunctionGrid(BuildContext context) {
    final functions = [
      ...
      {'icon': Icons.question_answer_outlined, 'title': '历史问卷'},
      {'icon': Icons.history_outlined, 'title': '历史公告'},
    ];

    final Map<String, void Function(BuildContext)> functionActions = {
      '家庭成员': (context) {
        // TODO: 跳转到家庭成员页面
        // Navigator.push(context, MaterialPageRoute(builder: (_) => FamilyPage()));
      },
      '家庭用车': (context) {
        // TODO: 跳转到家庭用车页面
      },
      ...
      '历史公告': (context) {
        NavigationService.navigateToHistoryCenter(context);
      },
    };
  ...
  Widget _buildFunctionItem(
    ...
    IconData icon,
    String title,
    Map<String, void Function(BuildContext)> functionActions,
  ) {
    return GestureDetector(
      onTap: () {
        final action = functionActions[title];
        if (action != null) {
          action(context);
        } else {
          // 可以弹个提示：功能开发中
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('$title 功能正在开发中')));
        }
      },
      child: Container(
        padding: EdgeInsets.symmetric(
